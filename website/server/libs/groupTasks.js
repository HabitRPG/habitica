...
async function _updateAssignedUsersTasks (masterTask, groupMemberTask) {
  if (groupMemberTask.type === 'todo') {
    ...
    // Completed or notDue dailies have little effect
    // at cron (MP replenishment for completed dailies)
    ...
    // @TODO Should we break their streaks or increase
    // their value to encourage competition for the daily?
    ...
    // For approval required tasks, this file treats approval
    // as completion for users other than the completing user
    // This protects other users who won't do the task if
    // the completing user decides to uncomplete after approval
    ...
    if (!approvalRequired) {
      ...
      taskDate = groupMemberTask.completed
        ? moment(groupMemberTask.history[groupMemberTask.history.length - 1].date)
        : taskDate;
    } else if (approved) {
    ...
    if (
      ...
      const historyIndex = masterTask.history
        .findIndex(masterEntry => masterEntry.date > historyEntry.date);
      if (historyIndex === -1) {
...
async function groupTaskCompleted (groupMemberTask, user, now) {
  ...
  if (masterTask.history && masterTask.history.length > 0) {
    ...
    // I couldn't think of a case where we would (User with a late day start in
    // Australia vs and early one in Hawaii?)
    ...
    // TODO When group scoring is implemented, there will be history entries
    // for group missed dailies, as well
...
async function groupTaskNewDay (groupMemberTask, user, now = moment()) {
  // Similar logic to groupTaskCompleted but returns update to uncomplete
  // group task if user day is *different* from task's last completed day
  const masterTask = await Tasks.Task.findOne({
    _id: groupMemberTask.group.taskId,
    type: 'daily',
    completed: true,
  ...
  if (!masterTask || !masterTask.group) return null;
