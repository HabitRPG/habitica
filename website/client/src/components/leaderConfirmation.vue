  ...
  <div class="member-details"
    ...
    <div class="member-stats"
      ...
      <div class="d-flex align-items-center profile-first-row">
        <class-badge v-if="classBadgePosition === 'next-to-name'"
        ...
        <div class="d-flex flex-column profile-name-character">
          <h3 class="character-name">
            ...
            <div v-if="isBuffed"
                 ...
                 class="is-buffed">
              <div class="svg-icon"
                   v-html="icons.buff"></div>
            </div>
          </h3>
          <div class="small-text character-level">
            <span v-if="member.auth && member.auth.local && member.auth.local.username"
                  class="mr-1">@{{ member.auth.local.username }}</span>
            <span v-if="member.auth && member.auth.local && member.auth.local.username"
                  class="mr-1">•</span>
...
export default {
  ...
  computed: {
    ...
    characterLevel () {
      return `${this.$t('level')} ${this.member.stats.lvl} ${this.member.stats.class ? this.$t(this.member.stats.class) : ''}`;
    },
    isBuffed () {
