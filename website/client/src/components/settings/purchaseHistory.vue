...
export default {
  ...
  async mounted () {
    ...
    this.gemTransactions = response.data.data.filter(transaction => transaction.currency === 'gems');
    this.hourglassTransactions = response.data.data.filter(transaction => transaction.currency === 'hourglasses');
  },
  methods: {
