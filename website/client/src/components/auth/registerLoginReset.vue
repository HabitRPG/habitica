<template>
  <div class="form-wrapper">
    ...
    <privacy-banner class="privacy-banner" />
    <form
    ...
    >
      ...
      <div class="form-group">
        <div>
          <div
            ...
            @click="proceed('apple')"
          >
            <div class="svg-icon social-icon" v-html="icons.appleIcon"></div>
            <div class="text">
      ...
      <div class="form-group" :class="{ 'mt-2': usernameIssues.length > 0 }">
      ...
      <div v-if="registering" class="form-group mb-4">
        <label v-once for="confirmPasswordInput">{{
          $t("confirmPassword")
        }}</label>
        <input
      ...
      <div class="text-center">
        ...
        <div>
          <router-link v-if="registering" :to="{ name: 'login' }" exact="exact">
            <a v-once class="white" v-html="$t('alreadyHaveAccountLogin')"></a>
          </router-link>
          <router-link
            v-if="!registering"
            :to="{ name: 'register' }"
            exact="exact"
          >
            <a v-once class="white" v-html="$t('dontHaveAccountSignup')"></a>
          </router-link>
        </div>
      </div>
    </form>
    <form v-if="forgotPassword" id="forgot-form" @submit.prevent="handleSubmit">
      <div>
        <div>
          <a
            href="/static/home"
    ...
    >
      ...
      <div class="form-group">
        <label v-once for="passwordInput">{{ $t("newPass") }}</label>
        <input
          id="passwordInput"
          v-model="password"
          class="form-control dark input-with-error"
      ...
      <div class="form-group mb-4">
        <label v-once for="confirmPasswordInput">{{ $t("confirmPass") }}</label>
        <input
          id="confirmPasswordInput"
          v-model="passwordConfirm"
          class="form-control dark input-with-error"
      ...
      <div class="text-center">
        <button
          ...
          :disabled="
            ...
            resetPasswordSetNewOneData.hasError
          "
  ...
  methods: {
    ...
    async resetPasswordSetNewOneLink() {
      ...
      const res = await axios.post(
        "/api/v4/user/auth/reset-password-set-new-one",
        {
