...
describe('POST /chat', () => {
  ...
  context("Describe Creating", () => {
    ...
    it('Returns an error when group is not found', () => {
      ...
      return expect(api.post('/groups/' + 'invalidID' + '/chat', { message: testMessage})).to.eventually.be.rejected.and.eql({
        code: 404,
        error: 'NotFound',
    ...
    it('Returns an error when chat privileges are provoked', () => {
      let groupName = 'Test Guild';
      let groupType = 'guild';
      let groupPrivacy = 'public';
      let testMessage = 'Test Message';
