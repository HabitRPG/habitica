...
import { predictableRandom } from './predictableRandomNamespaced';
...
module.exports = function randomDrop (user, modifiers, req = {}) {
  ...
  chance *= task.priority *
    (1 + (task.streak / 100 || 0)) *
    (1 + user._statsComputed.per / 100) *
    (1 + (user.contributor.level / 40 || 0)) *
    (1 + (user.achievements.rebirths / 20 || 0)) *
    (1 + (user.achievements.streak / 200 || 0)) *
  ...
  if (quest && quest.collect && predictableRandom(user) < chance) {
